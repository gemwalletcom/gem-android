...
class EvmTransactionStatusClient(
    ...
    private suspend fun getStatus(txId: String): TransactionChanges {
        ...
        val gasUsed = resp.gasUsed.hexToBigInteger() ?: return TransactionChanges(TransactionState.Pending)
        val effectiveGas = resp.effectiveGasPrice.hexToBigInteger() ?: return TransactionChanges(TransactionState.Pending)
        val l1Fee = resp.l1Fee?.hexToBigInteger() ?: BigInteger.ZERO
        val fee = gasUsed.multiply(effectiveGas) + l1Fee

        return TransactionChanges(state, fee)
    }
